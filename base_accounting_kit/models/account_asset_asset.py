...
class AccountAssetAsset(models.Model):
    ...
    category_id = fields.Many2one(
        "account.asset.category", string="Category", required=True, change_default=True
    )
    ...
    value_residual = fields.Float(
        compute="_amount_residual", digits=0, string="Residual Value"
    )
    ...
    def unlink(self):
        ...
        for asset in self:
            if asset.state in ["open", "close"]:
                raise UserError(
                    _("You cannot delete a document is in %s state.") % (asset.state,)
                )
            ...
            if self.prorata:
                ...
                if (
                    ...
                    depreciation_date = datetime.strptime(
                        str(self._get_last_depreciation_date()[self.id]), DF
                    ).date()
            else:
                ...
                if self.method_period >= 12:
                    if self.company_id.fiscalyear_last_month:
                        asset_date = (
                            ...
                            + relativedelta(year=int(self.date.year))
                        )  # e.g. 2018-12-31 +1 -> 2019
                    else:
                        asset_date = datetime.strptime(
                            str(self.date)[:4] + "-01-01", DF
                        ).date()
                else:
                    asset_date = datetime.strptime(
                        str(self.date)[:7] + "-01", DF
                    ).date()
                ...
                if (
                    ...
                    last_depreciation_date = datetime.strptime(
                        str(posted_depreciation_line_ids[-1].depreciation_date), DF
                    ).date()
                    depreciation_date = last_depreciation_date + relativedelta(
                        months=+self.method_period
                    )
    ...
    def validate(self):
        ...
        for asset in self:
            ...
            dummy, tracking_value_ids = asset._mail_track(
                tracked_fields, dict.fromkeys(field)
            )
            asset.message_post(
                subject=_("Asset created"), tracking_value_ids=tracking_value_ids
            )
        ...
        for asset in self:
            ...
            if unposted_depreciation_line_ids:
                ...
                sequence = (
                    ...
                    - len(unposted_depreciation_line_ids)
                    + 1
                )
    ...
    def create(self, vals):
        ...
        asset = super(
            AccountAssetAsset, self.with_context(mail_create_nolog=True)
        ).create(vals)
