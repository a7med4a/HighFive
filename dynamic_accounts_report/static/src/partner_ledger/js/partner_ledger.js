...
class PartnerLedger extends owl.Component {
    setup() {
        ...
        this.partnerId = params.partner_id || false;
        this.partnerName = params.partner_name || false;
        this.action_title = params.display_name || this.props.action.display_name || "Partner Ledger";
        this.action_xml_id = params.action_xml_id || '';
        this.state = useState({
            partners: null,
            data: null,
            total: null,
    ...
    async load_data() {
        ...
        let currency;
        var self = this;
        const action_title = this.action_title;
        try {
            var self = this;
            if (this.partnerName) {
                self.state.data = await this.orm.call(
                    "account.partner.ledger",
                    "get_filter_values",
                    [this.state.selected_partner, this.state.date_range, this.state.account, this.state.options, this.partnerName]
                );
            } else {
    ...
    async print_xlsx() {
        ...
        var action = {
            'data': {
                ...
                'output_format': 'xlsx',
                'report_action': self.props.action.xml_id || this.action_xml_id,
    ...
    async onSearchChange(ev) {
        ...
        let filtered_data = await this.orm.call("account.partner.ledger", "get_filter_values", [this.state.selected_partner, this.state.date_range, this.state.account, this.state.options, this.state.search_value]);
