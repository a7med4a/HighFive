...
class AccountRegisterPayments(models.TransientModel):
    ...
    effective_date = fields.Date(
        "Effective Date", help="Effective date of PDC", copy=False, default=False
    )
    ...
    def _create_payment_vals_from_batch(self, batch_result):
        ...
        the bank and cheque values"""
        res = super(AccountRegisterPayments, self)._create_payment_vals_from_batch(
            batch_result
        )
        if self.effective_date:
            res.update(
...
class AccountPayment(models.Model):
    ...
    def print_checks(self):
        ...
        selfs = self.filtered(
            lambda r: r.payment_method_id.code in ["check_printing", "pdc"]
        ...
        if not selfs[0].journal_id.check_manual_sequencing:
            ...
            last_printed_check = selfs.search(
                ...
                order="check_number desc",
                limit=1,
            )
            next_check_number = (
                last_printed_check and int(last_printed_check.check_number) + 1 or 1
            )
            return {
    ...
    def _prepare_payment_moves(self):
        ...
        inbound_pdc_id = self.env.ref(
            "base_accounting_kit.account_payment_method_pdc_in"
        ).id
        outbound_pdc_id = self.env.ref(
            "base_accounting_kit.account_payment_method_pdc_out"
        ).id
        if (
            self.payment_method_id.id == inbound_pdc_id
            or self.payment_method_id.id == outbound_pdc_id
