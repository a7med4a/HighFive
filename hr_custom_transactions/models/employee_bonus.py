...
class EmployeeBonus(models.Model):
    ...
    description = fields.Text(string='Reason for Bonus', required=True, tracking=True)
    ...
    total_value = fields.Monetary(string='Bonus Value', compute='_compute_total_value', store=True, currency_field='currency_id', tracking=True)
    ...
    # _sql_constraints = [
    #     ('unique_employee_bonus', 'unique(employee_id, request_date, description)', 'An employee bonus with the same employee, date, and description already exists!')
    # ]

    @api.depends('employee_id', 'bonus_type', 'unit_count')
    def _compute_total_value(self):
        for record in self:
            bonus_amount = 0.0
            if record.unit_count < 0:
                raise ValidationError('Count (Days/Hours) cannot be negative.')
            if record.bonus_type == 'days' and not record.employee_id.contract_id:
                raise ValidationError('Employee must have a contract to calculate day-based deductions.')
            elif record.bonus_type == 'days' and record.employee_id.contract_id:
                ...
                bonus_amount = record.unit_count * daily_wage
            elif record.bonus_type == 'hours':
                if record.employee_id.is_freelancer:
                    bonus_amount = record.unit_count * record.employee_id.hourly_rate
                    ...
                    bonus_amount = record.unit_count * hourly_wage
            record.total_value = bonus_amount
    ...
    def action_confirm(self):
        for record in self:
            if record.state == 'draft':
                ...
                raise UserError('Only draft bonuses can be confirmed.')
    ...
    def action_cancel(self):
        for record in self:
            if record.state in ('draft', 'confirmed'):
                ...
                raise UserError('Only draft or confirmed bonuses can be cancelled.')
    ...
    def action_set_to_draft(self):
        for record in self:
            if record.state == 'cancelled':
                ...
                raise UserError('Only cancelled bonuses can be set back to draft.')
