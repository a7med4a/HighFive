...
class AccountAssetDepreciationLine(models.Model):
    ...
    amount = fields.Float(string="Current Depreciation", required=True)
    remaining_value = fields.Float(string="Next Period Depreciation", required=True)
    ...
    move_posted_check = fields.Boolean(
        compute="_get_move_posted_check", string="Posted", store=True
    )
    ...
    def _get_move_posted_check(self):
        ...
        for each record in the 'AccountAssetDepreciationLine' class."""
        for line in self:
            line.move_posted_check = (
                True if line.move_id and line.move_id.state == "posted" else False
            )
    ...
    def create_move(self, post_move=True):
        ...
        for line in self:
            ...
            move_line_1 = {
                ...
                "debit": (
                    0.0
                    if float_compare(amount, 0.0, precision_digits=prec) > 0
                    else -amount
                ),
                "credit": (
                    amount
            ...
            for move_line in move.line_ids:
                if move_line.account_id.id == move_line_1["account_id"]:
                    move_line.write(
                        {"credit": move_line_1["credit"], "debit": move_line_1["debit"]}
                    )
                elif move_line.account_id.id == move_line_2["account_id"]:
                    move_line.write(
                        {"debit": move_line_2["debit"], "credit": move_line_2["credit"]}
                    )
    ...
    def create_grouped_move(self, post_move=True):
        ...
        category_id = self[
            0
        ].asset_id.category_id  # we can suppose that all lines have the same category
