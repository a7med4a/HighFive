...
class DeferredRevenueReport extends owl.Component {
    setup() {
        ...
        try {
            ...
            let filtered_data = await this.orm.call("deferred.report", "get_filter_values",
                [this.state.selected_journal_list, this.state.date_range, this.state.options,
                 this.state.selected_analytic_list, this.state.method, 'revenue']);

            this.state.journals = filtered_data['journal_ids'];
            this.state.analytics = filtered_data['analytic_ids'];

            let account_data = await this.orm.call("deferred.report", "view_deferred_revenue_report",
                [null, action_title]);
    ...
    // تأكد من أن البيانات في المكان الصحيح
    const reportData = {
        'account': this.state.account || [],
        'account_data': this.state.account_data || {},
        'total': this.state.account_total || {},
        'title': this.state.title || 'Deferred Revenue Report',
    ...
    return this.action.doAction({
        ...
        'report_name': 'dynamic_accounts_report.deferred_revenue_report',
        'report_file': 'dynamic_accounts_report.deferred_revenue_report',
        'data': reportData,  // البيانات هنا
        'context': reportData,  // وهنا أيضاً للأمان
    ...
    try {
        ...
        if (!this.state.account || this.state.account.length === 0) {
            alert('No data available to export. Please ensure you have deferred revenue entries.');
            return;
        }
        ...
        console.log('Sending XLSX data:', {
            accounts: reportData.account,
            account_count: reportData.account ? reportData.account.length : 0,
            has_account_data: !!reportData.account_data,
            has_totals: !!reportData.total,
            sample_account_data: reportData.account_data ? Object.keys(reportData.account_data).slice(0, 2) : []
        });

        // استخدام Cybrosys controller الموجود
        const action = {
            'model': 'deferred.report',
            'data': JSON.stringify(reportData),
            'output_format': 'xlsx',
            'report_name': this.state.title || 'Deferred Revenue Report',
            'report_action': this.props.action.xml_id || 'action_deferred_revenue_report'
        };
...
DeferredRevenueReport.defaultProps = {
    resIds: [],
};
DeferredRevenueReport.template = 'deferred_revenue_template';
actionRegistry.add("deferred_revenue_report", DeferredRevenueReport);