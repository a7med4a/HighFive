...
class AccountTrialBalance(models.TransientModel):
    ...
    def get_xlsx_report(self, data, response, report_name, report_action):
        ...
        if data["filters"]["account"]:
            account_keys = [
                account.get("display_name", "undefined")
        ...
        if data:
            if report_action == "dynamic_accounts_report.action_trial_balance":
                ...
                for move_line in data["data"][0]:
                    ...
                    sheet.write(
                        row, col + 1, move_line["initial_total_debit"], txt_name
                    )
                    sheet.write(
                        row, col + 2, move_line["initial_total_credit"], txt_name
                    )
                    j = 3
                    if data["apply_comparison"]:
                        ...
                        for num in number_of_periods:
                            sheet.write(
                                row,
                                col + j,
                                move_line["dynamic_total_debit_" + str(num)],
                                txt_name,
                            )
                            sheet.write(
                                row,
                                col + j + 1,
                                move_line["dynamic_total_credit_" + str(num)],
                                txt_name,
                            )
                            j += 2
                    ...
                    sheet.write(
                        row, col + j + 2, move_line["end_total_debit"], txt_name
                    )
                    sheet.write(
                        row, col + j + 3, move_line["end_total_credit"], txt_name
                    )
                    row += 1
        workbook.close()
        output.seek(0)
