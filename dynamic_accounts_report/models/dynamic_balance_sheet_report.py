...
class ProfitLossReport(models.TransientModel):
    ...
    account_ids = fields.Many2many(
        "account.account", string="Accounts", help="Select one or more accounts."
    )
        ...
        if comparison:
            for count in range(0, int(comparison) + 1):
                ...
                total_income = sum(
                    ...
                    for account_type in ["income", "income_other"]
                    for entry in account_entries[account_type][0]
                ) - sum(
                    float(entry["amount"].replace(",", ""))
                    for entry in account_entries["expense_direct_cost"][0]
                )
                total_expense = sum(
                    float(entry["amount"].replace(",", ""))
                    for account_type in ["expense", "expense_depreciation"]
                    for entry in account_entries[account_type][0]
                )
                ...
                total_assets = total_current_asset + sum(
                    float(entry["amount"].replace(",", ""))
                    for account_type in ["asset_fixed", "asset_non_current"]
                    for entry in account_entries[account_type][0]
                )
                ...
                total_liability = total_current_liability + sum(
                    float(entry["amount"].replace(",", ""))
                    for account_type in ["liability_non_current"]
                    for entry in account_entries[account_type][0]
                )
                total_unallocated_earning = (total_income - total_expense) + sum(
                    float(entry["amount"].replace(",", ""))
                    for account_type in ["equity_unaffected"]
                    for entry in account_entries[account_type][0]
                )
                total_equity = total_unallocated_earning + sum(
                    float(entry["amount"].replace(",", ""))
                    for account_type in ["equity"]
                    for entry in account_entries[account_type][0]
                )
    ...
    def _get_filter_data(self):
        ...
        analytic = [
            {"id": analytic.id, "name": analytic.name} for analytic in analytic_ids
        ]
    ...
    def get_xlsx_report(self, data, response, report_name, report_action):
        ...
        txt_name = workbook.add_format({'font_size': 13, })
        numbers = workbook.add_format({'font_size': 13, })
        txt_name_left = workbook.add_format(
            {'align': 'left', 'font_size': 15, }
        )
        txt_name.set_indent(4)
        numbers.set_indent(1)
        txt_name_left.set_indent(2)
        bottom_border_format = workbook.add_format(
            {'bottom': 2, 'bottom_color': '#000000', 'align': 'left', 'valign': 'vcenter', 'bold': True,
             'font_size': 15, })
        bottom_border_format_sub_heading = workbook.add_format(
            {'bottom': 2, 'bottom_color': '#000000', 'align': 'center', 'valign': 'vcenter', 'bold': True,
             'font_size': 15, })
        ...
        col = 0
        #company info
        sheet.merge_range(0, 0, 0, 1, 'Company name', company_txt)
        sheet.merge_range(1, 0, 1, 1, 'Address', company_txt)
        sheet.merge_range(2, 0, 2, 1, 'Vat', company_txt_last)

        sheet.merge_range(0, 2, 0, 5, company_name, company_txt)
        sheet.merge_range(1, 2, 1, 5, company_street, company_txt)
        sheet.merge_range(2, 2, 2, 5, company_vat, company_txt_last)

        sheet.merge_range(4, 0, 4, 1, report_name, sub_heading)
        sheet.write(5, col, '', sub_heading)
        for date in data['year']:
            sheet.write(4, col + 2, date, sub_heading)
            sheet.write(5, col + 2, 'Balance', sub_heading)
            col += 1
        col = 0
        if data:
            if report_action == 'dynamic_accounts_report.action_dynamic_profit_and_loss':
                sheet.merge_range(6, col, 6, 1, 'Net Profit', bottom_border_format)
                for datas in data['datas']:
                    sheet.write(6, col + 2, datas['total'], bottom_border_format)
                    col += 1
                col = 0
                sheet.merge_range(7, col, 7, 1, 'Income', bottom_border_format)
                sheet.write(7, col + 2, ' ', bottom_border_format)
                sheet.merge_range(8, col, 8, 1, 'Operating Income', txt_name_left)
                for datas in data['datas']:
                    sheet.write(8, col + 2, datas['income'][1], numbers)
                    col += 1
                row = 8
                index = 0
                for datas in data['datas']:
                    if index == 0:
                        for accounts in datas['income'][0]:
                            ...
                            if account_value == 1:
                                row += 1
                                col = 0
                                sheet.write(row, col, accounts['account_code'], txt_name)
                                sheet.write(row, col + 1, accounts['account_name'], txt_name)
                                for datas in data['datas']:
                                    for account in datas['income'][0]:
                                        if account_name == account['name']:
                                            sheet.write(row, col + 2, account['amount'], numbers)
                                            col += 1
                    index += 1
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Cost of Revenue', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['expense_direct_cost'][1], numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                    if index == 0:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Other Income', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['income_other'][1], numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                    if index == 0:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Total Income', side_heading_sub)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['total_income'], side_heading_sub)
                    col += 1
                row += 1
                col = 0
                sheet.merge_range(row, col, row, 1, 'Expense', bottom_border_format)
                sheet.write(row, col + 2, ' ', bottom_border_format)
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Expense', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['expense'][1], numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                    if index == 0:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Depreciation', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['expense_depreciation'][1],
                                numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Total Expenses', side_heading_sub)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['total_expense'], side_heading_sub)
                    col += 1
            else:
                sheet.merge_range(6, col, 6, 1, 'ASSETS', bottom_border_format_sub_heading)
                sheet.write(6, col + 2, ' ', bottom_border_format_sub_heading)
                sheet.merge_range(7, col, 7, 1, 'Current Assets', side_heading_sub)
                sheet.write(7, col + 2, ' ', side_heading_sub)
                sheet.merge_range(8, col, 8, 1, 'Bank and Cash Accounts', txt_name_left)
                for datas in data['datas']:
                    sheet.write(8, col + 2, datas['asset_cash'][1], numbers)
                    col += 1
                row = 8
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Receivables', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['asset_receivable'][1],
                                numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Current Assets', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['asset_current'][1],
                                numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Prepayments', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['asset_prepayments'][1],
                                numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Total Current Assets', side_heading_sub)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['total_current_asset'],
                                side_heading_sub)
                    col += 1
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Plus Fixed Assets', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['asset_fixed'][1],
                                numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Plus Non-current Assets', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['asset_non_current'][1],
                                numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Total Assets', side_heading_sub)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['total_assets'],
                                side_heading_sub)
                    col += 1
                col = 0
                row += 1
                sheet.merge_range(row, col, row, col + 1, 'LIABILITIES', bottom_border_format_sub_heading)
                sheet.write(row, col + 1, '', bottom_border_format_sub_heading)
                row += 1
                sheet.merge_range(row, col, row, col + 1, 'Current Liabilities', side_heading_sub)
                sheet.write(row, col + 1, ' ', side_heading_sub)
                row += 1
                sheet.merge_range(row, col, row, col + 1, 'Current Liabilities', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['liability_current'][1],
                                numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Payables', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['liability_payable'][1],
                                numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Total Current Liabilities', side_heading_sub)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['total_current_liability'],
                                side_heading_sub)
                    col += 1
                col = 0
                row += 1
                sheet.merge_range(row, col, row, col + 1, 'Plus Non-current Liabilities', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['liability_non_current'][1], numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                    if index == 0:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Total Liabilities', side_heading_sub)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['total_liability'],
                                side_heading_sub)
                    col += 1
                col = 0
                row += 1
                sheet.merge_range(row, col, row, col + 1, 'EQUITY', bottom_border_format_sub_heading)
                sheet.write(row, col + 2, '', bottom_border_format_sub_heading)
                row += 1
                sheet.merge_range(row, col, row, col + 1, 'Unallocated Earnings', side_heading_sub)
                sheet.write(row, col + 2, ' ', side_heading_sub)
                row += 1
                sheet.merge_range(row, col, row, col + 1, 'Current Earnings', txt_name)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['total_earnings'],
                                numbers)
                    col += 1
                col = 0
                row += 1
                sheet.merge_range(row, col, row, col + 1, 'Current Allocated Earnings', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['equity_unaffected'][1],
                                numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Total Unallocated Earnings', side_heading_sub)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['total_unallocated_earning'], side_heading_sub)
                    col += 1
                col = 0
                row += 1
                sheet.merge_range(row, col, row, col + 1, 'Retained Earnings', txt_name_left)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['equity'][1],
                                numbers)
                    col += 1
                index = 0
                for datas in data['datas']:
                ...
                row += 1
                col = 0
                sheet.merge_range(row, col, row, col + 1, 'Total EQUITY', side_heading_sub)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['total_equity'],
                                side_heading_sub)
                    col += 1
                col = 0
                row += 1
                sheet.merge_range(row, col, row, col + 1, 'LIABILITIES + EQUITY', side_heading_sub)
                for datas in data['datas']:
                    sheet.write(row, col + 2, datas['total_balance'],
                                side_heading_sub)
                    col += 1
        workbook.close()
        output.seek(0)
