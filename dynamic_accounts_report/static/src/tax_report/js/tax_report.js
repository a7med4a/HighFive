...
class TaxReport extends owl.Component {
    ...
    // إضافة Closing Entry function
    async onClosingEntry() {
        console.log('🔐 Creating Closing Entry...');
        try {
            const options = {
                date_from: this.start_date.el.value,
                date_to: this.end_date.el.value,
                company_ids: [1] // أو استخدم company ID الحالي
            };

            const result = await this.orm.call("tax.report", "action_periodic_vat_entries", [options]);
    ...
    // إضافة دالة لجعل الأرقام قابلة للنقر
    // إضافة دالة لجعل الأرقام قابلة للنقر مع error handling
    onClickAmount(ev, type, tax_name = null) {
        ev.preventDefault();
        console.log('🎯 Clicked amount:', type, tax_name);
...
//                    ['move_id.move_type', 'in', ['out_invoice', 'out_refund']],
                    ['date', '>=', this.start_date.el.value],
                    ['date', '<=', this.end_date.el.value]
                ];
                name = 'Sales Tax Journal Items';
            } else if (type === 'purchase_total') {
                domain = [
...
//                    ['move_id.move_type', 'in', ['in_invoice', 'in_refund']],
                ...
                name = 'Purchase Tax Journal Items';
            } else if (type === 'sale_tax' && tax_name) {
                domain = [
                    ['tax_ids.name', '=', tax_name],
                    ['tax_ids.type_tax_use', '=', 'sale'],
                    ['parent_state', '=', 'posted'],
//                    ['move_id.move_type', 'in', ['out_invoice', 'out_refund']],
                    ['date', '>=', this.start_date.el.value],
                ...
                name = `${tax_name} - Tax Amount`;
            } else if (type === 'purchase_tax' && tax_name) {
                domain = [
                    ['tax_ids.name', '=', tax_name],
                    ['tax_ids.type_tax_use', '=', 'purchase'],
//                    ['move_id.move_type', 'in', ['in_invoice', 'in_refund']],
                ...
                name = `${tax_name} - Tax Amount`;
            } else if (type === 'sale_net' && tax_name) {
                domain = [
                    ['tax_ids.name', '=', tax_name],
                    ['tax_ids.type_tax_use', '=', 'sale'],
//                    ['tax_line_id', '=', false],
...
//                    ['move_id.move_type', 'in', ['out_invoice', 'out_refund']],
                ...
                name = `${tax_name} - Base Amount`;
            } else if (type === 'purchase_net' && tax_name) {
                domain = [
                    ['tax_ids.name', '=', tax_name],
                    ['tax_ids.type_tax_use', '=', 'purchase'],
//                    ['tax_line_id', '=', false],
...
//                    ['move_id.move_type', 'in', ['in_invoice', '', 'in_refund']],
            ...
            if (domain.length > 0) {
                const action = {
                    ...
                    domain: domain,
                    target: 'current',
                    views: [[false, 'list'], [false, 'form']], // إضافة views
                ...
                console.error('❌ No domain defined for type:', type);
            }
        } catch (error) {
            console.error('❌ Error in onClickAmount:', error);
            this.notification.add('Error opening journal items: ' + error.message, {
                type: 'danger',
            });
        }
    }
    ...
    formatDate(dateString) {
        ...
        return date.toFormat('dd LLL yyyy'); // Example: "13 Oct 2025"
    }
    ...
    async applyFilter(val, ev, is_delete) {
        ...
        if (val && val.target.name === 'start_date') {
            ...
            this.start_date.el.value = today.startOf('month').toISODate();
            this.end_date.el.value = today.endOf('month').toISODate();
            this.state.date_viewed = []
            ...
            this.start_date.el.value = today.startOf('year').toISODate()
            this.end_date.el.value = today.endOf('year').toISODate()
            this.state.date_viewed = []
            ...
            this.start_date.el.value = today.startOf('quarter').toISODate()
            this.end_date.el.value = today.endOf('quarter').toISODate()
            this.state.date_viewed = []
            ...
            this.start_date.el.value = today.startOf('month').minus({ days: 1 }).startOf('month').toISODate()
            this.end_date.el.value = today.startOf('month').minus({ days: 1 }).toISODate()
            this.state.date_viewed = []
            ...
            this.start_date.el.value = today.startOf('year').minus({ days: 1 }).startOf('year').toISODate()
            this.end_date.el.value = today.startOf('year').minus({ days: 1 }).toISODate()
            this.state.date_viewed = []
            ...
            this.start_date.el.value = today.startOf('quarter').minus({ days: 1 }).startOf('quarter').toISODate()
            this.end_date.el.value = today.startOf('quarter').minus({ days: 1 }).toISODate()
            this.state.date_viewed = []
