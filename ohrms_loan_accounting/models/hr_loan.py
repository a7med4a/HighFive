...
class HrLoanAcc(models.Model):
    ...
    journal_id = fields.Many2one(
        ...
        default=lambda self: self.env['account.journal'].search([('is_loan', '=', True)], limit=1),
        domain=[('is_loan', '=', True)]
    )
    ...
    def action_approve(self):
        ...
        if loan_approve:
            ...
            timenow = self.date or date.today()
            for loan in self:
                ...
                debit_account_id = loan.employee_account_id.id  # الموظف مدين
                credit_account_id = loan.treasury_account_id.id  # البنك دائن

                debit_vals = {
                    'name': loan_name + ' ' + reference + ' ' +'سلفة',
                ...
                credit_vals = {
                    ...
                    'journal_id': journal_id,
                    'partner_id': loan.employee_id.work_contact_id.id if loan.employee_id.work_contact_id.id else '',
                    'date': timenow,
                    'debit': amount < 0.0 and -amount or 0.0,
                    'credit': amount > 0.0 and amount or 0.0,
                    'loan_id': loan.id,
    ...
    def action_view_moves(self):
        """Return action to view journal entries related to this loan"""
        return {
    ...
    def action_set_to_draft(self):
        for rec in self:
            ...
            # إذا يوجد قيد — امنع الإرجاع
            if moves:
                raise UserError("لا يمكن إرجاع السلفة إلى حالة المسودة لأن هناك قيد محاسبي مرتبط بها.")
...
class HrLoanLineAcc(models.Model):
    ...
    def action_paid_amount(self, month, date_to):
