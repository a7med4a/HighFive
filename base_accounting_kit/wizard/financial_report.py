...
class FinancialReport(models.TransientModel):
    ...
    def view_report_pdf(self):
        ...
        # checking view type
        return self.env.ref("base_accounting_kit.financial_report_pdf").report_action(
            self, data
        )
                    ...
                    if data["debit_credit"]:
                        ...
                        for company in account.company_ids:
                            if not company.currency_id.is_zero(
                                vals["debit"]
                            ) or not company.currency_id.is_zero(vals["credit"]):
                                flag = True
                    for company in account.company_ids:
                        if not company.currency_id.is_zero(vals["balance"]):
                            flag = True
                    if data["enable_filter"]:
                        vals["balance_cmp"] = value["comp_bal"] * int(report.sign)
                        for company in account.company_ids:
                            if not company.currency_id.is_zero(vals["balance_cmp"]):
                                flag = True
                    if flag:
