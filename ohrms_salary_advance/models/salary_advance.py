...
class SalaryAdvance(models.Model):
        ...
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == _('New'):
                vals['name'] = (
                        ...
                        .next_by_code('salary.advance.seq')
                        or _('New')
                )
    ...
    def approve_request_acc_dept(self):
        ...
        for request in self:
            move = {
                'name': 'Salary Advance Of' + ' ' + request.employee_id.name,
            ...
            if request.debit_id.id:
                debit_line = (0, 0, {
                    'name': request.employee_id.name + ' ' + request.name + ' ' + 'راتب مقدم',
            ...
            if request.credit_id.id:
                credit_line = (0, 0, {
                    ...
                    'partner_id': request.employee_id.work_contact_id.id if request.employee_id.work_contact_id.id else '',
                    'journal_id': request.journal_id.id,
                    'date': time.strftime('%Y-%m-%d'),
                    'debit': request.advance < 0.0 and -request.advance or 0.0,
                    'credit': request.advance > 0.0 and request.advance or 0.0,
