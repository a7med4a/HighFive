...
class AccountInvoiceLine(models.Model):
    ...
    asset_start_date = fields.Date(
        string="Asset Start Date", compute="_get_asset_date", readonly=True, store=True
    )
    asset_end_date = fields.Date(
        string="Asset End Date", compute="_get_asset_date", readonly=True, store=True
    )
            ...
            if cat:
                ...
                if record.move_id.invoice_date:
                    start_date = datetime.strptime(
                        str(record.move_id.invoice_date), DF
                    ).replace(day=1)
