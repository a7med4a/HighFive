...
class AccountJournal(models.Model):
    ...
    multiple_invoice_ids = fields.One2many(
        "multiple.invoice", "journal_id", string="Multiple Invoice"
    )
    ...
    text_position = fields.Selection(
        [("header", "Header"), ("footer", "Footer"), ("body", "Document Body")],
        required=True,
        default="header",
    ...
    text_align = fields.Selection(
        ...
        default="right",
        string="Center Align Text Position",
    )
    layout = fields.Char(
        string="Layout", related="company_id.external_report_layout_id.key"
    )
    ...
    def action_open_reconcile(self):
        ...
        if self.type in ("bank", "cash"):
            views = [
                ...
                (
                    self.env.ref(
                        "base_accounting_kit.account_bank_statement_line_view_tree"
                    ).id,
                    "list",
                ),  # Include tree view
            ]
            ...
            return {
                ...
                "search_view_id": [
                    self.env.ref(
                        "base_accounting_kit.account_bank_statement_line_view_search"
                    ).id,
                    "search",
                ],
