...
class ReportTax(models.AbstractModel):
    ...
    def get_lines(self, options):
        ...
        if options["date_from"] and not options["date_to"]:
            self.with_context(
                date_from=options["date_from"], strict_range=True
            )._compute_from_amls(options, taxes)
        elif options["date_to"] and not options["date_from"]:
            self.with_context(
                date_to=options["date_to"], strict_range=True
            )._compute_from_amls(options, taxes)
            ...
            self.with_context(date_to=date_to, strict_range=True)._compute_from_amls(
                options, taxes
            )
