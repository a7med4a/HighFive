...
class HrPayslip(models.Model):
    ...
    def action_payslip_confirm_records(self):
        """Function for change stage of Payslip"""
        for rec in self:
            if rec.state != 'draft':
                raise UserError(_('You can\'t confirmed a payslip which is\'t in draft state.'))
        for rec in self:
            rec.action_payslip_done()


    def action_payslip_compute_sheet_records(self):
        """Function for change stage of Payslip"""
        for rec in self:
            if rec.state != 'draft':
                raise UserError(_('You can\'t computed sheet a payslip which is\'t in draft state.'))
        for rec in self:
