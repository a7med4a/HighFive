...
class AccountMove(models.Model):
    ...
    def unlink(self):
        ...
        deferral_moves = self.filtered(
            ...
            and move.deferred_original_move_ids
        )
    ...
    def _compute_deferred_entry_type(self):
        for move in self:
            if move.deferred_original_move_ids:
                move.deferred_entry_type = (
                    "expense"
                    if move.deferred_original_move_ids[0].is_outbound()
                    else "revenue"
                )
            else:
    ...
    def _get_deferred_amounts_by_line(self, lines, periods, deferred_type):
        ...
        for line in lines:
            ...
            for period in periods:
                ...
                period_end = min(period[1], line_end) + relativedelta(
                    days=1
                )  # +1 to include  end date of report
                ...
                columns[period] = self._get_deferred_period_amount(
                    ...
                    line_start,
                    line_end
                    + relativedelta(days=1),  # +1 to include the end date of the line
                    line["balance"],
                )
    ...
    def _get_deferred_lines(
        ...
        return [
            ...
            for (account, coeff) in [
                (deferred_amounts[grouping_field], 1),
                (deferred_account, -1),
            ]
        ]
    ...
    def _generate_deferred_entries(self):
        ...
        for move_fully_deferred, lines_vals in zip(
            moves_fully_deferred, lines_vals_to_create
        ):
        ...
        for move_fully_deferred in moves_fully_deferred:
            ...
            deferred_move_ids = (
                move_fully_deferred.deferred_original_move_ids.deferred_move_ids
            )
            cancelling_moves = deferred_move_ids.filtered(
                lambda move: move_fully_deferred.date.replace(day=1)
                == move.date.replace(day=1)
    ...
    def open_deferred_entries(self):
        ...
        return {
            ...
            "views": [
                (
                    self.env.ref(
                        "deferred_revenues_expenses_community.view_deferred_entries_tree"
                    ).id,
                    "list",
                )
            ],
...
class AccountMoveLine(models.Model):
    ...
    move_attachment_ids = fields.One2many(
        "ir.attachment", compute="_compute_attachment"
    )
    ...
    def _has_deferred_compatible_account(self):
        ...
        return (
            ...
            in ("expense", "expense_depreciation", "expense_direct_cost")
        ) or (
            self.move_id.is_sale_document()
            and self.account_id.account_type in ("income", "income_other")
        )
    ...
    def _get_deferred_amounts_by_line_values(self, line):
        return {
            ...
            "product_id": (
                line["product_id"] if isinstance(line, dict) else line["product_id"].id
            ),
            "product_category_id": (
