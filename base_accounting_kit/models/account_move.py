...
class AccountMove(models.Model):
    ...
    asset_depreciation_ids = fields.One2many(
        "account.asset.depreciation.line", "move_id", string="Assets Depreciation Lines"
    )
    ...
    def action_post(self):
        ...
        for rec in self:
            if (
                rec.partner_id.active_limit
                and rec.move_type in pay_type
                and rec.partner_id.enable_credit_limit
            ):
                if (
                    rec.due_amount >= rec.partner_id.blocking_stage
    ...
    def check_due(self):
        ...
        if (
            self.partner_id
            and self.partner_id.due_amount > 0
