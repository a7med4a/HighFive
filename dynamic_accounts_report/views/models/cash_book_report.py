            ...
            if report_action == "dynamic_accounts_report.action_cash_book":
                ...
                for move_line in data["move_lines"]:
                    ...
                    for rec in data["data"][move_line]:
                        ...
                        sheet.merge_range(
                            row, col + 1, row, col + 2, rec["journal_id"][1], txt_name
                        )
                        sheet.merge_range(row, col + 3, row, col + 4, partner, txt_name)
                        sheet.merge_range(
                            row, col + 5, row, col + 6, rec["ref"], txt_name
                        )
                        sheet.merge_range(
                            row, col + 7, row, col + 8, rec["move_name"], txt_name
                        )
                        sheet.merge_range(
                            row, col + 9, row, col + 10, rec["name"], txt_name
                        )
                        sheet.merge_range(
                            row, col + 11, row, col + 12, rec["debit"], txt_name
                        )
                        sheet.merge_range(
                            row, col + 13, row, col + 14, rec["credit"], txt_name
                        )
