...
class AccountBankStatementLine(models.Model):
    ...
    def get_statement_line(self, record_id):
        ...
        statement_line_records = self.env["account.bank.statement.line"].search_read(
            [("id", "=", record_id)]
        )
        result_list = []
        for record in statement_line_records:
            ...
            if move_id:
                ...
                currency_symbol = self.env["res.currency"].browse(currency_id[0])
                ...
                self.env["account.bank.statement.line"].browse(record["id"]).write(
                    {"account_id": account_id.id}
                )
        return result_list
