...
class ReportCashBook(models.AbstractModel):
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account):
        ...
        if init_balance:
            ...
            filters = init_filters.replace("account_move_line__move_id", "m").replace(
                "account_move_line", "l"
            )
            sql = (
        ...
        # move lines
        sql = (
            """SELECT l.id AS lid, l.account_id AS account_id, l.date AS 
        ldate, j.code AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
            ...
            + """ GROUP BY l.id, 
                ...
                l.ref, l.name, m.name, c.symbol, p.name ORDER BY """
            + sql_sort
        )
        ...
        for account in accounts:
            ...
            currency = (
                ...
                or account_company.currency_id
            )
