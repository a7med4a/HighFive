...
class MultipleInvoiceLayout(models.TransientModel):
    ...
    multiple_invoice_type = fields.Selection(
        related="journal_id.multiple_invoice_type", readonly=False, required=True
    )
    text_position = fields.Selection(
        related="journal_id.text_position",
        readonly=False,
        required=True,
        default="header",
    )
    ...
    text_align = fields.Selection(related="journal_id.text_align", readonly=False)
    preview = fields.Html(
    ...
    @api.depends(
        "multiple_invoice_type", "text_position", "body_text_position", "text_align"
    )
